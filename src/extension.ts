...
import { inShell } from "./package-manager/src/chocolatey";
import {
  shellExec,
  inOsShell,
...
function isGitInstalled(): Thenable<boolean> {
  return inOsShell("git --version", {
    ...
    return vscode.commands
      .executeCommand("setContext", "git2go:isGitInstalled", isInstalled)
      .then(() => isInstalled);
  });
}
            ...
            if (success) {
              if (process.platform === "darwin") {
                ...
                vscode.window.showWarningMessage(
                  "Could not install git on windows. Install it manually."
                );
              }
...
function sshKeyDir(): string {
  ...
  if (process.platform === "darwin") {
    ...
    mySshKeyDir = execSync("echo %HOMEDRIVE%%HOMEPATH%\\.ssh")
      .toString()
      .trim();
  } else {
...
function hasSshKeys(): Thenable<boolean> {
  if (process.platform === "win32") {
    return inShell(`If (Test-Path -Path ${sshKeyDir()}\\id_rsa) { echo 1 }`, {
      disableChocoCheck: true,
    }).then((result) => {
    ...
    return shellExec(`test -f ${sshKeyDir()}/id_rsa && echo 1`).then(
      (result) => {
        if (result.success && result.msg === "1") {
          return true;
        }
  ...
  return hasSshKeys().then((hasKeys) => {
    if (hasKeys) {
      return new Promise((resolve) =>
        resolve(SuccessMsg("Already had SSH Keys"))
      );
    }
    return gitConfigGlobal("user.email").then((email) => {
      if (process.platform === "win32") {
        if (!fs.existsSync(sshKeyDir())) {
          fs.mkdirSync(sshKeyDir(), { recursive: true });
        }
        const cmd = `ssh-keygen -t rsa -C "${email}" -f ${sshKeyDir()}\\id_rsa -q -N '""'`;
        return inShell(cmd, { disableChocoCheck: true }).then((result) => {
          if (result.success) {
      ...
      return shellExec(unixCmd).then((result) => {
        if (!result.success) {
...
function gitConfigGlobal(property: string): Thenable<string> {
  return inOsShell(`git config --global ${property}`, {
    requiredCmd: "git",
    disableChocoCheck: true,
  }).then((result) => {
    if (result.success) {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  isGitInstalled().then((isInstalled) => {
    if (!isInstalled) {
      if (process.platform === "win32") {
        vscode.window.showWarningMessage(
          "Git is not installed. Install it manually and restart VS Code afterwards."
        );
      } else {
        vscode.window
          .showWarningMessage("Git is not installed", "Install now")
          .then((selection) => {
